# Keegan Moody - Profile

## Basic Information

**Name:** Keegan Moody
**Location:** Atlanta, Georgia
**Email:** keeganmoody33@gmail.com
**LinkedIn:** linkedin.com/in/keeganmoody33

---

## Professional Identity

**What I Do:** GTM Engineer — I build the infrastructure that makes revenue machines work.

**The Headline:** Builder. Hunter. Finds diamonds in dirt.

**One-Line:** Scientist-trained, blue-collar roots, sampling mentality. I build GTM systems that produce results.

---

## The Positioning

I'm a GTM Engineer — the person in the lab with the whiteboard, engineering the pieces that feed the flywheel. Prospecting systems, outbound automation, ICP frameworks, campaign architecture.

Think of it like building a car: I design the transmission, the headers, the exhaust — the systems that make the engine run. Then I hand it to the people who drive it.

**What makes me different:**
- Scientist-trained (organic chemistry, published research, 120 days fieldwork)
- Blue-collar roots (first-gen college student, worked with my hands)
- Sampling mentality from hip-hop production (diamonds from dirt, constraint-driven creativity)

I see patterns others miss because I learned to work with limitations and find the signal in the noise.

---

## Education

**Mercer University** — Macon, GA
BS Biochemistry and Molecular Biology (2019)
- Organic Chemistry SI Leader & Teaching Assistant
- Published researcher (2 Elsevier papers on mercury emissions)
- 120 days fieldwork in Peru

---

## The Story Arc

1. **Scientist** → Trained in organic chemistry, 120 days fieldwork in Peru, 2 published papers
2. **Medical school pivot** → Got accepted (Miami), walked away after seeing healthcare business up close
3. **Startup grind** → Biofourmis (co-founding SDR, North America, Orlando Health deal) → Trace Air ($220K in 90 days, 23 demos first month)
4. **Market intelligence** → Mobb AI (MCP early adoption, app security GTM, regulatory data targeting)
5. **GTM infrastructure** → Mixmax (built complete outbound system from zero, AI-native from day one)
6. **The builder** → Portfolio site, JD analyzer, tools — teaching myself to build what I imagine
7. **What's next** → Founding GTM role. Product-adjacent. Elite teammates. Long-term growth.

---

## Key Wins

| Achievement | Context |
|-------------|---------|
| $220K generated in 90 days | Trace Air — greenfield territory, self-sourced pipeline |
| 23 demos in first full month | 18 (78%) from contacts outside the CRM |
| Co-founding SDR for North America | Biofourmis — fed 6 AEs, enterprise healthcare |
| Orlando Health deal | $1M+ lifetime value, 90-day close, still active |
| Built GTM infrastructure from zero | Mixmax — 53 inboxes, ICP validation, full documentation |
| 120 days field research | Mercury research in Peru, 2 Elsevier publications |

---

## Key Quotes

### On Learning
> "Once I understood I could learn anything, accomplish anything, once I broke through that barrier, it opened up so many doors."

### On Approach
> "Give me the ability to hunt, cook, and clean my own food — I'll produce more than any SDR."

### On Creativity
> "Diamonds from dirt. Finding what's hidden in plain sight."

### On The Commitment
> "Ship first. Buzzer-beater second."

---

## Personal Elements

### The IROC-Z
- 1987 Camaro IROC-Z, bought for $5k from Indiana
- Full teardown and rebuild — frame off, body work, rust repair
- "You learn by taking things apart" → same approach to GTM systems

### The Music
- Sample-based production, hip-hop
- Technics SL-1200 MK2 turntables (one signed by Slick Rick)
- MPC 2000 XL
- Philosophy: "Diamonds from dirt. Finding what's hidden in plain sight."

### Camp Horizon
- Annual volunteer, foster children ages 8-12
- Only 1:1 camper-to-counselor ratio in the Southeast
- 4 years with same camper — "most influential male in this kid's life"

### Dr. Adam Kiefer (Mentor)
- "Nobody cares. Your complaints, your lack of responsibility — nobody cares. So stop."
- Took me to South America for 120 days
- First to show me there are things to be proud of
