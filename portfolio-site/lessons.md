...
- **Keep the turntable and YouTube iframe in the same page lifecycle.** The turntable entry is an overlay gate, not a separate route, so the already-mounted banner `YouTubePlayer` receives the user-gesture event and starts audio without unmounting the iframe. A route transition would risk killing playback before the banner player can persist it.
